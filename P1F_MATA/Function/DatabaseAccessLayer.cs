...
namespace P1F_MATA.Function
{
    ...
    public class DatabaseAccessLayer
    {
        ...
        // ===================================================================
        // ABN STORED PROCEDURES (DELETE)
        // ===================================================================

        /// Menghapus data ABN dari database.
        /// Hanya bisa dilakukan oleh user dengan level "superadmin".
        /// Proses penghapusan dilakukan dalam satu transaksi untuk memastikan konsistensi data:
        ///  1. Hapus data di tbl_mat_action (child table) yang berelasi dengan order_id yang akan dihapus.
        ///  2. Hapus data di tbl_mat_request (parent table) dengan order_id yang sama.
        /// Jika kedua operasi berhasil → commit transaksi dan kembalikan pesan sukses.
        /// Jika terjadi error atau data tidak ditemukan → rollback transaksi dan kembalikan pesan error
        /// yang sesuai (data tidak ditemukan vs error database).
        /// Parameter orderId adalah ID ABN yang akan dihapus, dan userLevel adalah level user yang mencoba melakukan penghapusan.
        /// Return: tuple (success, message) — success = true jika penghapusan berhasil, false jika gagal; message berisi informasi hasil operasi.

        public (bool success, string message) DeleteAbnormality(string orderId, string userLevel)
        {
            if (userLevel != "superadmin")
                return (false, "Access denied. Only superadmin can perform this action.");
            ...
            try
            {
                ...
                try
                {
                    // Hapus detail action dulu (child table)
                    using var cmd1 = new SqlCommand(
                        "DELETE FROM tbl_mat_action WHERE order_id = @order_id", conn, transaction);
                    cmd1.Parameters.AddWithValue("@order_id", orderId);
                    cmd1.ExecuteNonQuery();

                    // Hapus data induk (parent table)
                    using var cmd2 = new SqlCommand(
                        "DELETE FROM tbl_mat_request WHERE order_id = @order_id", conn, transaction);
                    ...
                    {
                        transaction.Commit();
                        return (true, $"Data {orderId} has been deleted successfully.");
                    }
                    else
                    {
